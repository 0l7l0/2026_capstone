...
다섯째, `final_judgment.py`의 C3 채점에 사용하던 입력이 `garch_gamma_results.csv`였으나, 이 파일은 본분석 노트북의 가공 결과로 직접 산출된 파일이 아닙니다. 본분석의 single source of truth인 `garch_model_params.csv`로 변경했습니다(2026-05-31). 두 파일의 GPR γ 값과 p값은 일치하므로 결과 자체는 동일하나, 검증의 추적성과 재현성이 강화됩니다. 추가로 동일 함수의 `c3_detail` 라벨에 "max p"라고만 표시하던 부분이 실제로는 `.min()`을 호출하던 버그가 있어 `min p`와 `max p`를 함께 표시하도록 수정했습니다. GPR γ 비유의 4개의 새 표시: `(n=4, min p=0.583, max p=0.960)`.
