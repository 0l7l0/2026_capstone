...
def tab_overview() -> None:
    ...
    .metric-card.red{
        border:1.8px solid #ff8787;
    }

    .metric-card.purple{
        border:1.8px solid #c0a0ff;
    }
    ...
    with c1:
        ...
        <div class="metric-card red">
            <div class="metric-title" style="color:#e03131;">최종 판정</div>
            <div class="metric-value" style="color:#e03131;">Weak Haven</div>
            <div class="metric-desc" style="color:#e03131;">❗Safe Haven 5/6 미충족</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with c2:
        ...
        <div class="metric-card purple">
            <div class="metric-title" style="color:#7950f2;">C1 · Event Study</div>
            <div class="metric-value" style="color:#7950f2;">PASS</div>
            <div class="metric-desc" style="color:#7950f2;">
                ✅ 이벤트 반응 확인
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with c3:
        ...
        <div class="metric-card green">
            <div class="metric-title" style="color:#099268;">C2 · Quantile Reg.</div>
            <div class="metric-value" style="color:#099268;">FAIL</div>
            <div class="metric-desc" style="color:#099268;">
                ❌ 5개 이벤트 미충족
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with c4:
        ...
        <div class="metric-card orange">
            <div class="metric-title" style="color:#e67700;">C3 · GARCH</div>
            <div class="metric-value" style="color:#e67700;">PASS</div>
            <div class="metric-desc" style="color:#e67700;">
                ✅ 변동성 안정성 확인
            </div>
        </div>
        """, unsafe_allow_html=True)
...
✅ C3(변동성): GPR 비유의 → 지정학 충격에 독립적 구조 확인

⚠️ C1/C2(수익률·분위수): 시장 급락 시 주식과 동조화 → 분산 기능 제한

🎯 BTC = **Weak Haven(조건부 안전자산)** — 시장 심리가 전쟁보다 강한 결정 변수
""")
...
def tab_event_study() -> None:
    ...
    event = pd.read_csv(ROOT / "validationv2/event_study_car_bh.csv")
    placebo = pd.read_csv(ROOT / "validationv2/event_study_placebo.csv")
    ...
    st.markdown("""
            <div style="background:#EFF6FF;border-left:5px solid #2563EB;padding:14px 18px;border-radius:6px;margin-top:8px;">
                <b>C1 판정:</b> BTC는 6개 이벤트 중 5개에서 양(+) CAR를 기록했으나,
                BH-FDR 보정 후 유의한 이벤트는 없었다.
                따라서 안전자산 방향성은 일부 확인되지만 Strong Safe Haven 증거는 확보되지 않았다.
            </div>
            """, unsafe_allow_html=True)
...
def run() -> None:
    ...
    # ── 페이지 제목 ──
    st.markdown(
        "<h1 style='font-size:38px;font-weight:800;margin-bottom:2px;'>₿ BTC 안전자산 가설 검증 대시보드</h1>"
        "<p style='font-size:15px;color:#9CA3AF;margin-bottom:0;'>"
        "</p>",
        unsafe_allow_html=True,
    )
    st.divider()
