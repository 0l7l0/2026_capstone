...
def tab_event_study() -> None:
    ...
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("BTC 양(+) CAR", "5 / 6")
    c2.metric("통계 유의 이벤트", "0 / 6")
    c3.metric("BH-FDR 보정 후", "전체 비유의")
    c4.metric("Gold 유의 이벤트", "1건 (BH전)")
    # 여기까지 미수정
    st.divider()
    ...
    <div style="background:#EFF6FF;border-left:5px solid #2563EB;padding:14px 18px;border-radius:6px;margin-top:8px;">
        <b>C1 판정:</b> BTC는 5/6 이벤트에서 양(+) CAR이나 통계 유의성 미확보 → C1 기준 미충족.
        Gold도 이스라엘-이란 이벤트 1건만 유의 (BH 보정 후 전체 비유의).
    </div>
    """, unsafe_allow_html=True)
    # 여기까지 미수정
