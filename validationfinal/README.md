...
### 정정 사항 (2026-05-28 ~ 05-31)

검증 과정에서 다섯 가지 누락·오류가 발견되어 정정되었습니다.

첫째, Ljung-Box 잔차 진단이 본분석 노트북에 코드는 있으나 실제 실행 결과가 출력되지 않은 채로 남아 있었습니다(noteook 셀 출력이 "잔차 진단 결과 없음"). `garch_conditional_volatility.csv`와 `garch_model_params.csv`에서 평균(Model3 mu=0.0885, 본진 기준)을 가져와 표준화 잔차에 직접 Ljung-Box를 적용한 결과를 `garch_ljung_box.csv`로 산출했습니다. 표준화 잔차는 lag 5·10·20 모두 자기상관 없음으로 통과했고(p=0.42·0.57·0.55), 잔차 제곱의 lag 5에서 ARCH 효과가 약하게 남았으나(p=0.0088) EGARCH 모형의 비대칭 효과가 이를 흡수합니다.
...
다섯째, `final_judgment.py`의 C3 채점에 사용하던 입력이 `garch_gamma_results.csv`였으나, 이 파일은 본분석 노트북의 가공 결과로 직접 산출된 파일이 아닙니다. 본분석의 single source of truth인 `garch_model_params.csv`로 변경했습니다(2026-05-31). 두 파일의 GPR γ 값과 p값은 일치하므로 결과 자체는 동일하나, 검증의 추적성과 재현성이 강화됩니다. 추가로 동일 함수의 `c3_detail` 라벨에 "max p"라고만 표시하던 부분이 실제로는 `.min()`을 호출하던 버그가 있어 `min p`와 `max p`를 함께 표시하도록 수정했습니다.

**GPR γ 4개 행 추출** (본진 `GARCH/result_csv_png/garch_model_params.csv`에서 `param`이 `gamma(...GPR...)` 인 행만 필터):

| model | model_label | variable | gamma | p_value |
|---|---|---|---:|---:|
| Model1 | 공식 GPR 단독 | GPR_zscore_scaled | -0.0106 | 0.8867 |
| Model2 | 커스텀 GPR 단독 | GPR_custom_scaled | +0.0319 | 0.7092 |
| Model4 | 공식 GPR + 시장심리 | GPR_zscore_scaled | +0.0259 | 0.7051 |
| Model5 | 커스텀 GPR + 시장심리 | GPR_custom_scaled | +0.0399 | **0.6066** |

→ n=4, min p=0.607, max p=0.887. 모두 p≥0.05 → C3 통과(`GPR γ 모두 비유의`). 옛 라벨 `max p=0.583`는 `.min()` 호출 결과(실제로는 최소 p)였고, 새 라벨은 `min p / max p` 둘 다 표시.

`final_judgment.csv`의 `C3_detail` 컬럼 6행 모두 다음과 같이 자동 생성됩니다: `GPR γ 모두 비유의 (n=4, min p=0.607, max p=0.887)`. 4개 행 추출 결과는 별도로 `garch_gpr_gamma_extraction.csv`에도 저장했습니다(source 컬럼에 본진 경로 명시).

> **2026-05-31 갱신**: 본 검증은 본진(`fbghkdrb`) 데이터 1800행 기준으로 일관 산출됩니다. 본진 `DataPipeline/processed_data/final/master_data.csv`가 1800행으로 멈춰있고 본분석 노트북들(EventStudy/Quantile/GARCH)이 모두 그 데이터로 실행된 상태입니다. 검증자 로컬에는 catalog v1.6 정본(1827행)이 있으나, 본진 결과와 매칭하기 위해 본진 1800행 데이터로 모든 검증 산출물(`final_judgment.csv`, `event_study_*.csv`, `multiple_testing_adjusted.csv`, `garch_ljung_box.csv`, `garch_gpr_gamma_extraction.csv`)을 재산출했습니다. 본분석 측이 1827행 정본 master_data로 EventStudy/Quantile/GARCH 노트북을 재실행하면 본 검증도 자동으로 1827행 기준으로 갱신 가능합니다(권고 사항).
...
| CAR 부호 | p_BH | C1_pass | C1_strength | 종합 표기 | 의미 |
|---|---|---|---|---|---|
| 양수 | < 0.05 (유의) | True | strong | **Pass (Strong)** | 강한 안전자산 |
| 양수 | ≥ 0.05 (비유의) | True | weak | **Pass (Weak\*)** | 부호 통과, 강도 약함 → 별표(\*) 표기 |
| 음수 | ≥ 0.05 (비유의) | False | inconclusive | **Fail (inconclusive)** | 부호 위반, 안전자산 증거 부족 |
| 음수 | < 0.05 (유의) | False | strong | **Fail (Risky)** | 유의한 음수, 위험자산 |

> **C1_pass와 C1_strength는 별도 컬럼**입니다. C1_pass(True/False)는 부호 기준 통과 여부만 표시하고, C1_strength(strong/weak/inconclusive)는 통계 강도를 표시합니다. 양수+비유의의 경우 C1_pass=True지만 C1_strength=weak이라 종합 표기에서 별표(\*)가 붙습니다. `final_judgment.csv`를 읽을 때 두 컬럼을 함께 봐야 정확한 의미가 전달됩니다.
...
> **p_BH 값이 6개 이벤트 모두 0.849로 동일한 이유**: BH-FDR 보정은 이벤트별이 아니라 family-wise(검정 집합 전체) 단위로 적용됩니다. 본 분석에서는 30개 검정(이벤트 6 × 자산 5)을 하나의 family로 묶었고, Benjamini-Hochberg step-up 절차에서 비유의 그룹은 동일한 보정값(0.849)으로 떨어집니다. 이는 BH 알고리즘 특성으로, 원본 p값 (`p_norm`)은 이벤트마다 다르나(0.107~0.481), 보정 후에는 비유의 그룹이 같은 값으로 묶이는 것입니다. 원본 p값을 함께 보려면 `event_study_car_bh.csv`의 `p_norm` 컬럼 참조.
...
| `garch_gpr_gamma_extraction.csv` | C3 채점 추적용 — `garch_model_params.csv`에서 GPR γ 4개 행만 추출 (Model1·2·4·5) |
| `garch_all_gamma_extraction.csv` | 본진 γ 전체 10행 (GPR 4 + VIX 3 + F&G 3) + `used_in_C3_scoring` / `category` 컬럼 — 검증자가 어느 행을 C3 채점에 썼는지 명시 추적 |
