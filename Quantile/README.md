...
* 결측치 제거
* Z-score 표준화 (독립변수: SP500, Gold, GPR_custom / 종속변수 BTC는 원래 로그수익률 그대로 사용)
* Min-Max 표준화 (강건성 검정)
* 상호작용항 생성
...
| 지정학 리스크 | γ     | GPR 효과                          |
| 상호작용    | δ     | Asset × GPR 효과                  |
| 표준오차    | HAC   | Newey-West 보정 (Bartlett 커널, 자동 대역폭) |
| 유효표본수   | eff_n | 분위수 구간 기대 관측수 (= n × τ)         |
...
```text
Qτ(BTC) = ατ + βτ · Asset_z + γτ · GPR_custom_z + ετ
```

> BTC는 로그수익률 원값, Asset_z · GPR_custom_z는 Z-score 표준화값
...
```text
Qτ(BTC) = ατ + βτ · Asset_z + γτ · GPR_custom_z + δτ · (Asset_z × GPR_custom_z) + ετ
```
...
(Baur & Lucey, 2010) — τ ≤ 0.10 구간에만 적용

| 조건                      | 판정                  |
| ----------------------- | ------------------- |
| β < 0 & p < 0.05        | ✅ Safe Haven        |
| β > 0 & p < 0.05        | ❌ Risky Asset       |
| β < 0 & 0.05 ≤ p < 0.10 | 🔵 Weak SH(10%)     |
| p ≥ 0.10                | ⚪ 비유의               |
| τ > 0.10                | — (판정 없음)           |
...
| 조건                         | 해석                          |
| -------------------------- | --------------------------- |
| β < 0 & p < 0.05, δ < 0 & p < 0.05 | ✅✅ 강한 Safe Haven     |
| β < 0 & p < 0.05, δ 비유의   | ✅ Safe Haven               |
| β 비유의, δ < 0 & p < 0.05   | 🔵 δ만 유의 (상호작용 Safe Haven) |
| β > 0 & p < 0.05           | ❌ Risky Asset              |
| p ≥ 0.05                   | ⚪ 상호작용 효과 없음              |
...
| Step 1 | 데이터 로드                                    |
| Step 2 | 표준화 (Z-score)                             |
| Step 3 | 전체 + 이벤트별 분위수 회귀 (SP500+GPR / Gold+GPR)  |
| Step 4 | 상호작용항 분위수 회귀 (SP500+GPR+IA / Gold+GPR+IA) |
| Step 5 | 시각화 (β 경로 · γ 경로 · 이벤트×분위수 히트맵)          |
| Step 6 | 최종 판단 — Safe-Haven 판정                     |
| Step 7 | 결과 저장                                     |
| Step 8 | 강건성 검정 (3종)                               |
| Step 9 | 전체 결과 저장                                  |
...
```text
SP500 모델 결론 유지
Gold 모델 일부 민감성 존재 (호르무즈·솔레이마니 제외 시 일부 비유의 전환)
```
...
| 0.05 | +0.01766 | <0.001  | Risky Asset |
| 0.10 | +0.01575 | <0.001  | Risky Asset |
| 0.50 | +0.01099 | <0.001  | 평상시 기준     |

### 핵심 해석

```text
주식이 급락할수록 BTC도 함께 하락
→ Safe Haven 아님 → Risky Asset
```
...
```text
지정학 리스크 자체가 BTC 수익률을 직접 설명하지 못함
```
...
BTC는 모든 주요 이벤트, 모든 극단 하락 분위수, 모든 강건성 검정에서 Safe Haven으로 확인되지 않음.

오히려 SP500과 유의한 양(+)의 관계를 유지하며 Risky Asset 성격을 보였다.
