...
> 이전 채점 로직이 "음수 CAR도 비유의면 C1 통과"로 처리하던 분기 제거. Baur & Lucey (2010) 원래 정의 (부호 기준)에 일관 적용. 이스라엘-하마스(CAR=-0.029)·이스라엘-이란 충돌(CAR=-0.063)이 음수 CAR로 C1 미달 → Weak Haven에서 Diversifier로 강등.
