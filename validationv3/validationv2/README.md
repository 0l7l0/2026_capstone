...
**GPR γ 4개 행 추출** (`garch_model_params.csv`에서 `param`이 `gamma(...GPR...)` 인 행만 필터):

| model | model_label | variable | gamma | p_value |
|---|---|---|---:|---:|
| Model1 | 공식 GPR 단독 | GPR_zscore_scaled | -0.0045 | 0.9602 |
| Model2 | 커스텀 GPR 단독 | GPR_custom_scaled | +0.0263 | 0.7346 |
| Model4 | 공식 GPR + 시장심리 | GPR_zscore_scaled | +0.0440 | **0.5833** |
| Model5 | 커스텀 GPR + 시장심리 | GPR_custom_scaled | +0.0364 | 0.5962 |

→ n=4, min p=0.583, max p=0.960. 모두 p≥0.05 → C3 통과(`GPR γ 모두 비유의`). 옛 라벨 `max p=0.583`는 `.min()` 호출 결과(실제로는 최소 p)였고, 새 라벨은 `min p / max p` 둘 다 표시.

`final_judgment.csv`의 `C3_detail` 컬럼 6행 모두 다음과 같이 자동 생성됩니다: `GPR γ 모두 비유의 (n=4, min p=0.583, max p=0.960)`. 4개 행 추출 결과는 별도로 `garch_gpr_gamma_extraction.csv`에도 저장했습니다.
