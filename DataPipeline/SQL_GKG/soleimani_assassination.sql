WITH base AS (
  ...
  WHERE
    ...
    _PARTITIONTIME >= TIMESTAMP('2019-09-24')
    AND _PARTITIONTIME <= TIMESTAMP('2021-01-28') 
...
filtered AS (
  SELECT 
    'soleimani_assassination' AS event_label,
    *
  FROM parsed
  WHERE
    (
    ...
    AND 
    (
          LOWER(persons) LIKE '%soleimani%'
          OR LOWER(organizations) LIKE '%irgc%'
          OR LOWER(organizations) LIKE '%quds%'
          OR LOWER(locations) LIKE '%baghdad%'
