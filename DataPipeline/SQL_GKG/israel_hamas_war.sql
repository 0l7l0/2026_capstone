WITH base AS (
  ...
  WHERE
    ...
    _PARTITIONTIME >= TIMESTAMP('2022-12-17')
    AND _PARTITIONTIME < TIMESTAMP('2024-01-03') 
...
filtered AS (
  SELECT 
    'israel_hamas_war' AS event_label,
    *
  FROM parsed
  WHERE
    (
    ...
    AND (
          LOWER(locations) LIKE '%israel%'
          OR LOWER(locations) LIKE '%gaza%'
          OR LOWER(locations) LIKE '%palestin%'
