"""catalog.json 기반 자동 검증 — 3종 본분석 (event_study / garch_x / quantile_reg).
...
RED_FLAG_PATTERNS = {
    ...
    'bootstrap': {
        '소표본(n<200) Block 단독 사용 (Wild 미병행)': {
            'must_exist': [r'wild[_\s]?bootstrap|Wild|Rademacher|rademacher|mammen'],
            'severity': 'WARN',
        },
    },
    'quantile_reg': {
...
def write_markdown_report(reports: list[dict], cycle: int, out_path: Path) -> None:
    ...
    _cat_ver = json.load(open(CATALOG, encoding='utf-8')).get("_meta", {}).get("version", "?")
    lines.append(f'- catalog: `.claude/references/catalog.json` (v{_cat_ver})')
...
def main():
    ...
    targets = ['event_study', 'garch_x', 'quantile_reg']
    if args.methodology:
