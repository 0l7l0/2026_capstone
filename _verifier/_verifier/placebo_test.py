...
v1.6 추가: 데이터 시작점 BUFFER 내 이벤트(호르무즈)는 자동으로 forward-only (이벤트 이후만)
        적용. 양방향 후보 부족(n=0) 문제 해결. 사유: catalog `forward_placebo_when`.

산출: Edit_mj/results/event_study_placebo.csv
컬럼: event_name, real_CAR, placebo_mean_CAR, placebo_std_CAR, percentile_of_real, placebo_p,
      n_placebo, placebo_mode (bidirectional / forward_only)
"""
import sys
...
def compute_car(ret_df: pd.DataFrame, anchor: pd.Timestamp,
    ...
    # v1.6: 데이터 시작점 근접 이벤트(호르무즈) — est_start_d NaT 시 첫 거래일로 fallback
    # 본분석 노트북도 max(0, ei+est_start) 동일 처리
    if pd.isna(est_start_d):
        est_start_d = dates.iloc[0]
    if pd.isna(est_end_d) or pd.isna(evt_start_d) or pd.isna(evt_end_d):
        return None
...
def main():
    ...
    for event, ed in EVENT_DATES.items():
        ...
        # v1.6: 데이터 시작점 BUFFER 내 이벤트는 forward-only (catalog `forward_placebo_when`)
        forward_only = (ev_pos < min_pos + BUFFER)
        if forward_only:
            candidates = [i for i in range(excl_hi + 1, max_pos + 1)]
            placebo_mode = 'forward_only'
        else:
            candidates = [i for i in range(min_pos, max_pos + 1) if i < excl_lo or i > excl_hi]
            placebo_mode = 'bidirectional'
        ...
        if len(placebo_cars) == 0:
            ...
            rows.append({'event_name': event, 'real_CAR': real_car, 'placebo_mean_CAR': None,
                         'placebo_std_CAR': None, 'percentile_of_real': None,
                         'placebo_p': None, 'n_placebo': 0, 'placebo_mode': placebo_mode})
            continue
        ...
        mode_tag = '[fwd]' if placebo_mode == 'forward_only' else '[bi]'
        print(f'  {event} {mode_tag}: real_CAR={real_car:+.4f} | placebo_mean={placebo_cars.mean():+.4f}±{placebo_cars.std():.4f} | percentile={pct:.3f} | p={p_two:.4f} | n={len(placebo_cars)}')
