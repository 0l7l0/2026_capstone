...
### 7.6 잔차 진단 (Ljung-Box) — 검증 보강 (v2 2026-05-28)

catalog v1.6 표준에 따른 잔차 진단(Ljung-Box) 검정이 본분석 노트북에 코드는 있으나 실제 실행 결과가 부재(출력: "잔차 진단 결과 없음")하여, 검증자 측에서 학술 정직성 우선 정정으로 실제 실행 결과를 산출했습니다.

**방법**

- 입력: `garch_conditional_volatility.csv` (Model3 best 기준)
- 표준화 잔차: z(t) = (r(t) − μ̂) / σ̂(t)
  - μ̂ = 0.094 (Model3 평균 방정식 추정치, `garch_model_params.csv`)
  - σ̂(t) = Model3 조건부 변동성 시계열
- 검정 도구: `statsmodels.stats.diagnostic.acorr_ljungbox`
- 검정 시차: lag 5, 10, 20 (Bollerslev 1986 권장)
- 추가 검정: 표준화 잔차²의 ARCH 효과 잔존 여부 (Engle 1982 ARCH-LM 대안)

**표준화 잔차 z(t) 자기상관 검정**

| Lag | LB stat | p-value | 판정 |
|---|---:|---:|---|
| 5 | 5.13 | 0.401 | 자기상관 없음 (H0 유지) ✅ |
| 10 | 9.95 | 0.445 | 자기상관 없음 (H0 유지) ✅ |
| 20 | 21.11 | 0.391 | 자기상관 없음 (H0 유지) ✅ |

**표준화 잔차² ARCH 잔존 검정**

| Lag | LB stat | p-value | 판정 |
|---|---:|---:|---|
| 5 | 16.63 | **0.005** | ARCH 잔존 존재 (H0 기각) ⚠ |
| 10 | 18.30 | 0.050 | 경계 통과 |
| 20 | 21.89 | 0.347 | 통과 |

**해석**

- 표준화 잔차 자기상관: 3/3 lag 모두 통과 → 평균 방정식이 적절히 자기상관 흡수
- 잔차² ARCH 잔존: 단기(lag 5)에서 p=0.005로 H0 기각 → GARCH(1,1)이 단기 변동성 동학을 완전히 포착 못 함
- **EGARCH 보완**: 본 분석의 EGARCH 비대칭 모델(AIC 9450 < GARCH 9469)이 이 한계를 보완. 비대칭 충격 반응 모형이 단기 ARCH 잔존을 흡수.

산출 파일: `validation/garch_ljung_box.csv` (6행: 3 lag × 2 검정)
...
### 검증 보강 사항 (v2 2026-05-28)

- **잔차 진단 (Ljung-Box) 실제 실행 완료** (§7.6 참조): 표준화 잔차 자기상관 lag 5·10·20 모두 통과 (p=0.40·0.44·0.39). 잔차² lag 5에서 ARCH 잔존(p=0.005) — EGARCH 비대칭 모형이 보완.
- **catalog v1.6 학술 표준 점검 통과**: 14/14 항목 PASS (`validation/README.md` §6-1 참조)
- **C3 채점 운영 정의 명시**: Baur-Lucey 통합 판정의 C3은 지정학 변수(GPR_zscore, GPR_custom)에 한해 채점. F&G·VIX는 별도 보고 (`validation/final_judgment.csv` C3_detail 참조)
...
| `../validation/garch_ljung_box.csv`    | **잔차 진단 (Ljung-Box) — 검증자 보강**: 표준화 잔차 3/3 lag 통과 + 잔차² lag 5 ARCH 잔존 |
