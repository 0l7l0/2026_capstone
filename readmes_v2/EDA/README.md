...
## 8. 검증 반영 (v2 2026-05-31)

본 EDA 결과는 `validation/` 폴더의 학술 표준 점검(catalog v1.6)에 사용되었습니다.

| 검증 항목 | 결과 | 영향 |
|---|---|---|
| ARCH 효과 검정 | 6개 자산 모두 p<0.05로 기각 | GARCH 모델 적용 정당화 (catalog v1.6 GARCH-X 전제 조건 충족) |
| ADF 정상성 검정 | 6개 자산 모두 p<0.05로 기각 | 시계열 분석 정당화 (단위근 없음) |
| Jarque-Bera 정규성 | 전원 기각 | Student-t 분포 사용 정당화 (GARCH 오차항) |

**본분석 6개 이벤트 확장**: 본 EDA는 5개 이벤트(COVID-19 포함)로 진행했으나, 본분석(EventStudy/Quantile/GARCH)은 6개 지정학 이벤트(호르무즈 위기 추가, COVID-19 제외)로 확장됨. COVID-19는 지정학 이벤트가 아니므로 본분석 범위에서 의도적 제외 (`catalog.json` `scope_note` 참조).
