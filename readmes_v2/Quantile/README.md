...
### 이벤트 라벨 정합성 (2026-05-29 정정 명시)

| 코드 키 (master_data) | CSV 저장 라벨 (quantile_results_bh.csv) | 의미 |
|---|---|---|
| `hormuz_crisis` | 호르무즈 위기 | 2019-06-13 |
| `soleimani_assassination` | 솔레이마니 암살 | 2020-01-03 |
| `russia_ukraine_war` | 러-우 전쟁 | 2022-02-24 |
| `israel_hamas_war` | 이스라엘-하마스 | 2023-10-07 |
| `israel_iran` | 이스라엘-이란 충돌 | 2024-04-14 |
| `us_israel_iran` | **이란 전쟁** | **2026-02-28 (= 미-이스라엘-이란)** |

> ⚠ **라벨 정합성**: `us_israel_iran` 이벤트가 분위수 회귀 결과 CSV에 **'이란 전쟁'** 라벨로 저장됨. 메인 README 섹션 4 종합 판정 (`미-이스라엘-이란`)과 매칭 시 별칭으로 처리됨 (`validation/` 자동 검증 도구가 매핑 처리). 향후 라벨 통일 권장 (`us_israel_iran` → '미-이스라엘-이란'으로 통일).
...
**Safe-Haven 판정 기준 (Baur & Lucey 2010, 부호 기준 엄격 적용)**

| 조건 | 판정 |
|------|------|
| **β ≤ 0**, p < 0.05 (τ ≤ 0.10) | ✅ Safe Haven (강도 강함) |
| **β ≤ 0**, p ≥ 0.05 (τ ≤ 0.10) | Weak non-pos (단독 C2 통과, 종합 판정 별도) |
| **β > 0**, p ≥ 0.05 | 비유의 양수 → **C2 미달** (Baur-Lucey 부호 기준) |
| **β > 0**, p < 0.05 | ❌ Risky Asset |
| δ < 0, p < 0.05 | GPR 상승 시 Safe Haven 성격 강화 |
| δ > 0, p < 0.05 | GPR 상승 시 위험자산 동조화 강화 |

> ⚠ **2026-05-29 정정**: 이전 판정 표는 "p ≥ 0.05 → 비유의" 단일 분류였으나, Baur-Lucey 부호 기준 적용 시 β 부호 (양수/음수)별로 분류가 다름. 양수 β는 비유의여도 C2 미달.
...
**이벤트별 Safe-Haven 판정 (τ = 0.05, SP500_z β 기준)**

| 이벤트 | β (SP500) | p_BH | C2 판정 | 종합 판정 (final_judgment) |
|--------|-----------|------|---------|---|
| 전체 합산 | +0.016 | <0.001 | ❌ 위험자산 | — |
| 호르무즈 위기 | **-0.012** | 0.254 | ✅ Weak non-pos | Safe Haven\* (강도 약함) |
| 솔레이마니 암살 | +0.014 | <0.001 | ❌ 위험자산 | Weak Haven |
| 러-우 전쟁 | +0.032 | <0.001 | ❌ 위험자산 | Weak Haven |
| 이스라엘-하마스 | +0.013 | 0.005 | ❌ 위험자산 | **Diversifier** (C1 미달, CAR=-0.029) |
| 이스라엘-이란 충돌 | +0.024 | <0.001 | ❌ 위험자산 | **Diversifier** (C1 미달, CAR=-0.063) |
| 미-이스라엘-이란 (CSV: '이란 전쟁') | +0.015 | 0.003 | ❌ 위험자산 | Weak Haven |
...
> **핵심 결론**
> - **BTC는 전체 합산에서 모든 위기 분위수에서 ❌ Risky Asset** — 주식 폭락 시 BTC도 함께 하락
...
> - Gold 모델은 호르무즈·솔레이마니 이벤트 제외 시 판정이 변하여 SP500 모델보다 민감함
> - **호르무즈만 β=-0.012로 부호 통과** (단 p=0.254 비유의 → Weak non-pos)
> - **이벤트별 종합 판정은 메인 README 섹션 4 / `Dashboard/result_csv_png/final_judgment.csv` 참조** (Safe Haven\* 1 / Weak 3 / Diversifier 2)
...
| `result_csv_png/quantile_results_bh.csv` | CSV | BH 다중검정 보정 적용 결과 (us_israel_iran은 '이란 전쟁'으로 저장됨) |
